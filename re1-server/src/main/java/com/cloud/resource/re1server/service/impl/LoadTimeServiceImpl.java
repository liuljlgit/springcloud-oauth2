package com.cloud.resource.re1server.service.impl;
...
import com.cloud.resource.re1server.service.inft.ILoadTimeService;
import com.cloud.resource.re1server.dao.inft.ILoadTimeDao;
import com.cloud.resource.re1server.entity.LoadTime;
import com.cloud.resource.re1server.cache.inft.ILoadTimeRedis;
import com.cloud.resource.re1server.webentity.LoadTimeResp;
...
public class LoadTimeServiceImpl implements ILoadTimeService{
    ...
    public Integer insertLoadTime(LoadTime loadTime) throws Exception {
        if(Objects.isNull(loadTime)){
            return 0;
        }
        ...
        Integer result =  loadTimeDao.insertLoadTime(loadTime);
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void insertLoadTimeList(List<LoadTime> loadTimeList) throws Exception {
        ...
        loadTimeDao.insertLoadTimeList(loadTimeList);
        loadTimeRedis.deleteAllHashSetByPage();
    }
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public Integer deleteLoadTimeByKey(Long ltId) throws Exception {
        if(Objects.isNull(ltId)){
            throw new BusiException("请输入要删除的数据的ID");
        }
        Integer result = loadTimeDao.deleteLoadTimeByKey(ltId);
        loadTimeRedis.deleteAllHashSetByPage();
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void deleteLoadTimeList(List<LoadTime> list) throws Exception {
        if(CollectionUtils.isEmpty(list)){
            return ;
        }
        for (LoadTime loadTime : list) {
        ...
        loadTimeDao.deleteLoadTimeList(list);
        loadTimeRedis.deleteAllHashSetByPage();
    }
    ...
    public List<LoadTime> findLoadTimeListByIds(List<Long> list,Boolean useCache) throws Exception {
        ...
        if(useCache){
            resList = loadTimeRedis.getLoadTimeListByIds(list);
            Map<Long, LoadTime> loadTimeMap = resList.stream().collect(Collectors.toMap(e -> e.getLtId(), e -> e));
            List<Long> nullList = list.stream().filter(e -> !loadTimeMap.containsKey(e)).collect(Collectors.toList());
            if(CollectionUtils.isEmpty(nullList)){
                return resList;
            }else{
                List<LoadTime> nullObjList = loadTimeDao.findLoadTimeListByIds(nullList);
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void saveLoadTime(LoadTime loadTime,Boolean isFullUpdate) throws Exception {
        if(Objects.isNull(loadTime)){
           return ;
        }
        if(Objects.isNull(loadTime.getLtId())){
            loadTime.setLtId(loadTimeRedis.getLoadTimeId());
            insertLoadTime(loadTime);
        }else{
            updateLoadTime(loadTime,isFullUpdate);
        }
    }
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void saveLoadTimeList(List<LoadTime> loadTimeList,Boolean isFullUpdate) throws Exception {
        if(CollectionUtils.isEmpty(loadTimeList)){
            return ;
        }
        List<LoadTime> insertList = loadTimeList.stream().filter(e -> Objects.isNull(e.getLtId())).collect(Collectors.toList());
        ...
        if(!CollectionUtils.isEmpty(insertList)){
            ...
            insertLoadTimeList(insertList);
        }
        if(!CollectionUtils.isEmpty(updateList)){
            updateLoadTimeList(updateList,isFullUpdate);
        }
    }
}
